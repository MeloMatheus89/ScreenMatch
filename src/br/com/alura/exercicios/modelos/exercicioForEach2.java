...
public class exercicioForEach2 {
    public static void main(String[] args) {

        Cachorro novoDog = new Cachorro();
        novoDog.setNome("Novo Dog");
        novoDog.setPeso(12.00);
        novoDog.setIdade(6);

        Animal passarinho = new Animal();
        passarinho.setNome("Kass");

        ArrayList<Animal> listaDeAnimais = new ArrayList<>();
        listaDeAnimais.add(novoDog);
        listaDeAnimais.add(passarinho);

        for (Animal item: listaDeAnimais){
            System.out.println("Vamos falar de um animal chamado: "+item.getNome());
            if (item instanceof Cachorro) {
                System.out.println("Esse cachorro pesa: "+((Cachorro) item).getPeso()+ "kg");
            }
        }
